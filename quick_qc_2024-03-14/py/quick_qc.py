...
def read_config(): #Read in taget parameters and settings from config
    ...
    \033[1mSource\033[0m
        - Source path: /ags/projects/hiaper/code_noah/python/quick_qc/quick_qc_2024-03-14/py/quick_qc.py
        - Author: Noah Barton nbarton@ucsd.edu
        - Last Modified: 2024-03-14 nbarton, added switches
    ...
    global start_dir, campaign, receiver, aircraft, A, B, r2_threshold, out_filename_prefix, flight_ids, show_bad
    ...
    receiver = config['receiver'] #nobody cares anymore
    aircraft = config['aircraft']
    flight_ids = config['flight_ids']
    show_bad = config['show_bad']
...
def read_profiles(): #load desired profiles to perform quick QC on into a Pandas DataFrame
    ...
    if (flight_ids == 'all') or (flight_ids == ''):
        if campaign[0:2] == 'TC':
            year = campaign[2:]
            print(os.path.join(start_dir,'20'+year+'*'))
            iops = glob.glob(os.path.join(start_dir,'20'+year+'*'))
        if campaign[0:2] == 'AR': #get both fall and spring flights for 1 water year
            year1 = campaign[2:]
            year2 = str(int(year1) - 1)
            iops = glob.glob(os.path.join(start_dir,'20'+year2+'*'+receiver)) + glob.glob(os.path.join(start_dir,'20'+year1+'*'+receiver))
        iops = sorted(iops)
    else:
        iops = []
        for flight_id in flight_ids:
            iops.append(glob.glob(os.path.join(start_dir,flight_id)))
        iops = sorted(np.array(iops).flatten().tolist())
    profiles = pd.DataFrame(columns=['flight','occ','lat','lon','msl_alt','ref','log_ref', 'grad','d2n','msg','bad'])
    ...
    for iop in iops: #start by loading all profiles into Pandas df
        print(iop)
        occultations = sorted([occultation for occultation in os.listdir(iop) if '-' in occultation])
        n_occs = len(occultations)
        n_ncs = 0
        n_good = 0
        for occultation in occultations:
            ncs = [filename for filename in os.listdir(os.path.join(iop,occultation)) if '_nc' in filename]
            if len(ncs) != 1:
                continue
            n_ncs += 1
            if (len(occultation) > 9) & (show_bad == False):
                continue
            n_good += 1
            try:
                with nc.Dataset(os.path.join(iop,occultation,ncs[0])) as ds:
                    lat = ds['Lat'][:] 
                    lon = ds['Lon'][:]
                    msl_alt = ds['MSL_alt'][:]
                    ref = ds['Ref'][:]
                ...
                bad = len(occultation) > 9
                profiles.loc[len(profiles)] = [os.path.basename(iop), occultation, lat, lon, msl_alt, ref, log_ref, grad, d2n, None, bad]
            except:
                continue
        profiles_by_flight.loc[len(profiles_by_flight)] = [os.path.basename(iop), n_occs, n_ncs, n_good]
a_s = np.empty(0)
b_s = np.empty(0)
r2_s = np.empty(0)
...
def run_qc(): #performs quick QC on loaded profiles and generates text results
    ...
    profiles_savelist = profiles[['flight','occ','msg']]
    print(out_filename_prefix+'_profiles_savelist.xlsx')
    profiles_savelist.to_excel(out_filename_prefix+'_profiles_savelist.xlsx')
    ...
    with open(out_filename_prefix+'_results.txt', 'w') as f:
        ...
        f.write('Mean summary statistics: '+'\n')
        f.write(profiles_by_flight[profiles_by_flight.p_good > 0].mean().to_string()+'\n')
        '''
        if aircraft == 'G-IV':
...
def static_plot(): #make a refractivity plot of all profiles
    ...
    if (show_bad == False): profiles = profiles[profiles.bad == False]
